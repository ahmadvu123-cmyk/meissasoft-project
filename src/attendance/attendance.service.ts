...
export class AttendanceService {
    ...
    async getAllAttendances(query: GetAttendanceDto) {
        const { worker_id, page = 1, limit = 10, start_date, end_date, search } = query;
        ...
        const attendances = await this.attendanceRepo.findMany(skip, limit, whereCondition);
        const totalAttendances = await this.attendanceRepo.countTotalAttendances(whereCondition);
